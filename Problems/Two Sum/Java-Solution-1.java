...
class Solution {
    public int[] twoSum(int[] nums, int target) {
        ...
        for (int i = 0 ; i < nums.length ; i++) {
            if (numsMap.containsKey(target - nums[i]) && numsMap.get(target - nums[i]) != i) { // Checking if the difference exists inside the HashMao and if it's not the same element
